...
std::vector<std::string> m_deny_from_clients;
...
/* Aayush */
std::string m_snatched_buffer;
std::string m_cheat_log_buffer;
std::string m_active_torrent_buffer;
/* Aayush */
...
time_t m_read_db_deny_from_clients_time;
...
static bool banned_client(const std::array<char, 20>& m_peer_id)
{
	std::string peer_id(std::begin(m_peer_id), std::end(m_peer_id));
...
static void async_query(const std::string& v, bool async = true)
{
...
void read_config()
{
	if (m_use_sql)
	{
		try
		{
			...
			m_database.set_name("deny_from_hosts", m_table_prefix + "deny_from_hosts");
			m_database.set_name("deny_from_clients", m_table_prefix + "deny_from_clients");
			m_database.set_name("cheat_log", m_table_prefix + "cheat_log");
			m_database.set_name("snatched", m_table_prefix + "snatched");
		}
		catch (bad_query&)
		{
		}
...
void read_db_torrents_sql()
{
	try
	{
		if (!m_config.m_auto_register)
        {
            ...
            Csql_result result = Csql_query(m_database, "select info_hash, @fid, flags, freeleech from @files where flags & 1").execute();
            while (Csql_row row = result.fetch_row())
            {
            	if (row[2].i() == 1)
            	{
            ...
            {
            	buffer.pop_back();
            	Csql_query(m_database, "update @files set flags = 0 where @fid IN (" + buffer + ")").execute();
            }
        }
		...
		Csql_result result = Csql_query(m_database, "select info_hash, @completed, @fid, ctime, freeleech, @uid from @files where @fid >= ?")(m_fid_end).execute();
		...
		while (Csql_row row = result.fetch_row())
		{
			...
			file.fid = row[2].i();
			file.ctime = row[3].i();
			file.freeleech = (row[4].i() ? true : false);
			file.tor_user_id = row[5].i();
			file.dirty = false;
		}
	}
	catch (bad_query&)
...
void read_db_deny_from_clients()
{
	...
	try
	{
		Csql_result result = Csql_query(m_database, "select peer_id from @deny_from_clients").execute();
...
void write_db_torrents()
{
	...
	if (!m_announce_log_buffer.empty())
	{
		...
		async_query("insert delayed into " + db_name("announce_log") + " (ipa, port, event, info_hash, peer_id, downloaded, left0, uploaded, uid, mtime) values " + m_announce_log_buffer, false);
		m_announce_log_buffer.erase();
	}
	if (!m_scrape_log_buffer.empty())
	{
		m_scrape_log_buffer.pop_back();
		async_query("insert delayed into " + db_name("scrape_log") + " (ipa, uid, mtime) values " + m_scrape_log_buffer, false);
...
int test_sql()
{
	...
	try
	{
		...
		Csql_query(m_database, "select @fid, info_hash, @leechers, @seeders, flags, mtime, ctime, freeleech, @uid from @files where 0").execute();
		Csql_query(m_database, "select @fid, @uid, active, announced, completed, downloaded, `left`, uploaded, up_rate, down_rate, useragent, peer_id, ipa, seedtime, leechtime from @files_users where 0").execute();
		
		if (m_config.m_log_scrape)
			Csql_query(m_database, "select id, ipa, uid, mtime from @scrape_log where 0").execute();
		
		Csql_query(m_database, "select @uid, torrent_pass_version, downloaded, uploaded, seedbonus from @users where 0").execute();
		Csql_query(m_database, "select @uid, @fid, mtime, ipa from @snatched where 0").execute();
		...
		m_read_users_freeleech = Csql_query(m_database, "show columns from @users like 'freeleech'").execute();
		return 0;
	}
	catch (bad_query&)
	{
...
{
	try {
		Csql_query(m_database, "TRUNCATE @files_users").execute();
	}
	catch(bad_query&) {
		std::cerr << "Unable to truncate peers table!" << std::endl;
	}
}
/* @Aayush */
...
std::string srv_insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	{	
		...
		if (update_at) {
			m_active_torrent_buffer += Csql_query(m_database, "(?, ?, ?),")
				(user->uid)
		...
		/* @Aayush */ 
		if (m_config.m_freeleech || file.freeleech || user->freeleech)	{
		...
		if (downloaded || uploaded || seedbonus) {
			m_users_updates_buffer += Csql_query(m_database, "(?,?,?,?),")(downloaded)(uploaded)(user->uid)(seedbonus).read();
		}

		if (completed_torrent)
		{
			m_snatched_buffer += Csql_query(m_database, "(?, ?, ?, ?),")
				(user->uid)
				(srv_time())
				(file.fid)
				(ntohl(v.m_ipa))
...
/* @Aayush */
std::string srv_update_tracker(boost::unordered_map<std::string, std::string> &params)
{		
	if (params["key"] != m_config.m_torrent_pass_private_key)
	{
	...
	else if(params["action"] == "stats") 
	{
		...
		params.emplace("seeders", std::to_string(seeders));
		params.emplace("leechers", std::to_string(leechers));
		params.emplace("torrents", std::to_string(torrents));
		params.emplace("snatches", std::to_string(snatches));
...
std::string write_json(const std::string &message)
{
...
std::string write_json(const boost::unordered_map<std::string, std::string> &params)
{
...
std::string srv_statistics()
{
    ...
    os << "<tr><td>"
            ...
            << "<tr><td>read db files time<td class=ar>" << t - m_read_db_torrents_time << " / " << m_config.m_read_db_files_interval;
    if (m_use_sql)
    {
            os << "<tr><td>read db users time<td class=ar>" << t - m_read_db_users_time << " / " << m_config.m_read_db_users_interval
